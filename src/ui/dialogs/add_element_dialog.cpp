...
struct AddElementDialog::Impl {
    ...
    // If external parser provided, we do not own it
    JsonParser* m_externalParser = nullptr;
    // Internal parser used when no external parser is provided
    JsonParser m_internalParser;
};

AddElementDialog::AddElementDialog(QWidget* parent)
    : QDialog(parent), m(new Impl()) {
    m->m_externalParser = nullptr;
...
AddElementDialog::AddElementDialog(JsonParser* sharedParser, QWidget* parent)
    : QDialog(parent), m(new Impl()) {
    m->m_externalParser = sharedParser;
    setWindowTitle("Add Element");
    setModal(true);
    resize(400, 200);
...
void AddElementDialog::onLoadJsonClicked() {
    ...
    JsonParser* parser = m->m_externalParser ? m->m_externalParser : &m->m_internalParser;
    if (!parser->loadJsonFile(filePath)) {
        // show error
        QMessageBox::critical(this, "Error", QString("Failed to load JSON: %1").arg(parser->getLastError()));
        return;
    }
